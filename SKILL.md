...
- Before one-by-one native GPU submissions, run `hpc_resource_planner.py` and follow only its `next_action`; pass `--available-children N` or `--child-manifest children.json` before allowing 3-8GPU candidates. Use `--test-only-probe --probe-script candidate.template.sbatch --write-selected-script candidate.selected.sbatch` for exact-script preflight evidence. Without `--probe-script`, planner probes are resource-shape-only and the exact sbatch script still needs preflight.
...
- For normal GPU training submissions through native Slurm, force `--gres-flags disable-binding` and choose the largest same-node CPU/GPU shape supported by the monitor resource snapshot. Use `16` CPU cores per training task only when the snapshot shows a non-reserved node can satisfy it; otherwise start with `12`, then `8`. Treat `8` CPU cores per training task as the ordinary minimum. For native packed `2GPU` jobs only, if `2GPU/16CPU` is still blocked by `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure, test emergency `2GPU/8CPU` with `--ntasks=2 --cpus-per-task=4 --gres=gpu:2` before giving up or switching accounts. If the user explicitly prioritizes occupying GPUs and native same-node evidence shows a usable fragment with `free_cpu >= 2 * requested_gpu` but not enough CPU for ordinary shapes, use one native GPU-fill allocation for the whole fragment, for example `--nodes=1 --ntasks=4 --cpus-per-task=2 --gres=gpu:4 --gres-flags=disable-binding`, only when `--available-children N` or a child manifest proves there are enough independent one-GPU children and a matching N-child sbatch script has been generated. If the packed 2GPU shape still cannot be scheduled but a native 1GPU singleton can run and the child experiments are single-GPU capable, split the packed pair into native 1GPU singleton jobs. Do not request more than `16` CPU cores per task unless the user explicitly asks for a diagnostic probe or a high-CPU override.
- Before choosing CPU/GPU parameters for queued follow-ups, refills, or authorized pending replacements, use the live monitor resource snapshot from `hpc_queue_summary.py --json`. Inspect `checked_at_local`, `cluster_resources.summary`, `cluster_resources.nodes`, `cluster_resources.excluded_reserved_nodes`, account summaries, pending reasons, and job `resources`. If the monitor/widget payload is stale, missing `cluster_resources`, or reports a resource query error, refresh it before selecting a shape. A `2GPU` shape fits only when one non-reserved node has `gpu_free >= 2` and `cpu_free >= 2 * cpus_per_task`; a `1GPU` shape fits only when one non-reserved node has `gpu_free >= 1` and `cpu_free >= cpus_per_task`; GPU-fill fits only when explicit GPU-first mode is active and one non-reserved node has enough same-node CPU for `2` CPUs per requested GPU. Do not blindly submit `2GPU/32CPU` queued jobs when the snapshot already shows no node can satisfy that shape; preselect the largest currently fitting shape, then let `sbatch --test-only` be the final authority. When no same-node candidate fits, treat planner `queue_probe` output as backlog guidance, not an immediate submit recommendation; prefer history-supported `2GPU/8CPU` or `2GPU/16CPU` backlog over high-CPU `2GPU/24CPU`/`2GPU/32CPU` queue probes unless the user explicitly requests CPU-rich queued work. If visible free GPUs have too little same-node CPU even for `1GPU/4CPU` or GPU-fill, classify the blocker as same-node CPU exhaustion.
...
Use the builder for all wide/GPU-fill allocations so requested GPU/task count equals child command count. Use native submit without `--submit` for read-only exact-script preflight; add `--submit` only after preflight succeeds.
...
1. Request one batch allocation with the required GPU count, `--gres-flags=disable-binding`, and enough CPU for all child experiments. Use the monitor resource snapshot to start from the largest same-node shape that currently fits rather than defaulting to an oversized queued request. For two single-GPU children, prefer `--gres=gpu:2`, `--ntasks=2`, and `--cpus-per-task=16` only when a non-reserved node has at least 2 free GPUs and 32 free CPUs. Otherwise try `--cpus-per-task=12`, then `8`. Do not go below 8 CPU cores per child for ordinary packed jobs. If native `2GPU/16CPU` is still blocked by `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure, an emergency packed fallback may use `--cpus-per-task=4`, giving 4 CPU cores per child. If the user explicitly selects GPU-first fragment mode and same-node Slurm evidence shows `free_gpu >= 3` plus `free_cpu >= 2 * free_gpu` but not enough CPU for ordinary/emergency shapes, use one native allocation for the whole fragment, for example `--ntasks=4 --cpus-per-task=2 --gres=gpu:4`, and launch one independent single-GPU child per task. Do not submit separate `1GPU/2CPU` jobs for this mode. If the packed 2GPU shape still cannot be scheduled but native `1GPU/8CPU` can run, split the independent child experiments into singleton jobs and retry each singleton with `--cpus-per-task=8`, then emergency `4`.
...
- If a packed job is already native `2GPU/16CPU` with no dependency, node pin, or feature constraint, re-submitting the same shape is not a repair. If the blocker is `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure, an authorized replacement may test emergency `2GPU/8CPU` (`--cpus-per-task=4`) while preserving the child experiment labels and parameters. If the job is already emergency `2GPU/8CPU` and visible free GPUs have too little same-node CPU even for `1GPU/4CPU` or GPU-fill, replacement is not a repair; preserve queue position and report `SchedNodeList`, `StartTime`, and `LastSchedEval`.
- If GPU-first fragment mode is active, an authorized replacement may combine one or more same-account same-project pending packed jobs into a single fragment allocation that claims all usable GPUs on the target node with `--cpus-per-task=2`; lower-priority pending jobs may be included only when every canceled child is preserved in the replacement.
...
- GPU-fill fragment jobs are distinct from singleton fallback: they are single Slurm allocations for multiple one-GPU children, may use `2` CPUs per child only when explicitly requested and recorded, and should claim the whole usable same-node GPU fragment.
...
- Submit each CPU/GRES-sensitive GPU training job as a native Slurm script with explicit project and Python paths, `--gres-flags=disable-binding`, and the largest same-node CPU/GPU shape that the monitor resource snapshot currently shows as feasible. For singleton jobs, use `--ntasks=1 --cpus-per-task=16` only when a non-reserved node has at least 1 free GPU and 16 free CPUs; otherwise start with `12`, then the ordinary minimum `8`. For packed `2GPU` jobs blocked by `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure at `2GPU/16CPU`, an authorized replacement may retry emergency `2GPU/8CPU` with `--cpus-per-task=4`. If the pending job is already emergency `2GPU/8CPU` and no same-node `1GPU/4CPU` or GPU-fill candidate fits, do not cancel/re-submit just to chase visible GPUs; preserve queue position and report same-node CPU exhaustion. If the packed 2GPU shape still cannot schedule but 1GPU can run and the child experiments are single-GPU capable, split to native `1GPU/8CPU` singleton jobs with `1GPU/4CPU` as emergency fallback. Make the job name encode the experiment and slot.
...
- If `QOSMaxJobsPerUserLimit` blocks two single-GPU run slots for one account, prefer one native packed job with `--gres=gpu:2 --gres-flags=disable-binding`, but still choose its CPU shape from the monitor resource snapshot. Use `16` CPU cores per child only when a non-reserved same node has enough free CPU/GPU; otherwise use `12`, then the ordinary minimum `8`. If `2GPU/16CPU` remains blocked by `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure, test emergency `2GPU/8CPU` with `4` CPU cores per child before giving up. If no same-node candidate fits, treat high-CPU planner `queue_probe` results as backlog guidance only and prefer lower history-supported shapes unless the user requests CPU-rich queued work. If packed 2GPU still cannot schedule but 1GPU singleton preflight passes, split the pair into singletons. Pack only two experiments per ordinary allocation unless GPU-fill fragment mode is explicitly recorded or the user explicitly approves more.
...
- For CPU/GRES-sensitive GPU training, use native Slurm, force `--gres-flags=disable-binding`, and use the monitor resource snapshot to choose the largest same-node shape that currently fits. Use `16` CPU cores per training task only when the snapshot supports it; otherwise fall back to `12`, then `8`. For packed `2GPU` jobs blocked by `Resources`, reservation constraints, node CPU availability, or another resource-shape allocation failure, emergency `2GPU/8CPU` with `4` CPU cores per child is allowed after `2GPU/16CPU` fails. If no same-node candidate fits, planner `queue_probe` output is backlog guidance, not an immediate submit recommendation; do not submit high-CPU `2GPU/24CPU` or `2GPU/32CPU` solely from a no-fit snapshot. If the user explicitly prioritizes GPU utilization and same-node Slurm evidence supports GPU-fill fragment mode, submit one allocation for the whole fragment, such as `--ntasks=4 --cpus-per-task=2 --gres=gpu:4`, with one independent single-GPU child per task. If packed 2GPU still cannot schedule but a 1GPU singleton can run, split single-GPU-capable children into `1GPU/8CPU` singleton jobs with `1GPU/4CPU` as emergency fallback. If a pending packed job is already emergency `2GPU/8CPU` and visible GPUs have too little same-node CPU even for GPU-fill, preserve queue position and report same-node CPU exhaustion. Do not go below `8` CPUs per child for ordinary training jobs, below `4` CPUs per child for emergency packed or 1GPU compatibility jobs, or below `2` CPUs per child for explicitly recorded GPU-fill fragment jobs.
